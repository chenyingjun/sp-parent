...
import com.chenyingjun.sp.shiro.CustomShiroSessionDAO;
import com.chenyingjun.sp.shiro.bo.UserOnlineBo;
...
public class CustomSessionManager {
	 ...
	 * session status 
	 */
	public static final String SESSION_STATUS ="meeting-online-status";
	...
	public List<SimplePrincipalCollection> getSimplePrincipalCollectionByUserId(Long...userIds){
		...
		for (Session session : sessions) {
			...
			if(null != obj && obj instanceof SimplePrincipalCollection){
				...
				if(null != obj && obj instanceof SystemUser){
	...
	private UserOnlineBo getSessionBo(Session session){
		...
		if(obj instanceof SimplePrincipalCollection){
			...
			if(null != obj && obj instanceof SystemUser){
				//存储session + user 综合信息
				UserOnlineBo userBo = new UserOnlineBo((SystemUser)obj);
				//最后一次和系统交互的时间
				...
				//session最后一次与系统交互的时间
				userBo.setLastTime(session.getLastAccessTime());
				//回话到期 ttl(ms)
